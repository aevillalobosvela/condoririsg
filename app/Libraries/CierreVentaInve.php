...
class CierreVentaInve extends CierreVentaBasePdf
{
    ...
    public function generarReporteContadoMatrix(array $reportData, array $filters): void
    {
        ...
        foreach ($ventasAgrupadas as $venta) {
            ...
            foreach ($productosUnicos as $prod => $info) {
                if (isset($venta['items'][$prod])) {
                    ...
                    $this->Cell($subColW, 5, $q > 0 ? $q : '', 1, 0, 'C');
                    $this->Cell($subColW, 5, $bs > 0 ? number_format($bs, 0) : '', 1, 0, 'R');
                } else {
