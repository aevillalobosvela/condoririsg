...
<div class="container-fluid">
  ...
  <div class="header-top mb-4">
    ...
    <div class="nav-buttons">
      ...
      <a href="<?= base_url('resecciones') ?>" class="btn btn-success btn-sm">
  ...
  <div class="pos-container">
            ...
            <div class="col-6 col-md-4 product-card" 
              ...
              <div class="card h-100 shadow-sm border-0" style="border-left: 4px solid <?= $border ?> !important;">
                <div class="card-body text-center p-3" style="background: linear-gradient(135deg, <?= $bg ?> 0%, #ffffff 100%);">
                  ...
                  <div class="mb-2">
                    <span class="badge" style="background-color: <?= $border ?>; font-size: 0.65rem;"><?= $p->stock_inve ?> <?= esc($p->unidad ?? 'und') ?></span>
                  </div>
    ...
    <div class="summary-sidebar">
      ...
      <form id="ventaForm" method="post" action="<?= base_url('inventarios/guardarCreditoVenta') ?>">
        ...
        <div class="total-summary-card">
          ...
          <div class="payment-section mt-4 pt-3 border-top">
           ...
           <a href="<?= base_url('inventarios/ventas') ?>" class="btn btn-outline-danger w-100" id="cancelSaleBtn">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // ✅ AHORA USAMOS stock_inve en lugar de stock
    const allProducts = <?= json_encode(array_map(function($p) {
        return [
            'id' => $p->id,
            'nombre' => $p->nombre,
            'precio_contado' => (float)$p->precio_contado,
            'stock' => (int)$p->stock_inve,   // ✅ CORREGIDO: stock_inve
            'unidad' => $p->unidad ?? 'und'
        ];
    }, $productos)) ?>;
    ...
    function fetchSaldoCredito(tipo, id) {
      fetch(`<?= base_url('inventarios/saldoCredito') ?>?tipo=${tipo}&id=${id}`)
        .then(r => r.json())
        .then(data => {
    ...
    dipSearch.addEventListener('input', function() {
      ...
      searchTimeout = setTimeout(() => {
        fetch(`<?= base_url('inventarios/buscarPersonalUto') ?>?dip=${encodeURIComponent(dip)}`)
    ...
    document.getElementById('btnGuardarExterno').addEventListener('click', function() {
      ...
      fetch('<?= base_url('inventarios/guardarClienteExterno') ?>', {
        method: 'POST',
    ...
    function renderProducts(nameFilter = '', categoryFilter = '') {
      ...
      productsGrid.innerHTML = paginated.map(p => {
        const colors = getProductColors(p.nombre);
          ...
          <div class="card h-100 shadow-sm border-0 cursor-pointer" style="border-left: 4px solid ${colors.border} !important;">
            <div class="card-body text-center p-3" style="background: linear-gradient(135deg, ${colors.bg} 0%, #ffffff 100%);">
              <div class="rounded-circle d-flex align-items-center justify-content-center mx-auto mb-2 overflow-hidden" 
                ...
                <span class="fw-bold" style="font-size: 0.9rem; color: white; display: none; width: 100%; height: 100%; align-items: center; justify-content: center;">${p.nombre.substring(0,2)}</span>
              </div>
              
              <h6 class="card-title mb-2" style="color: ${colors.text}; font-size: 0.8rem; line-height: 1.2;">${p.nombre}</h6>
    ...
    function renderCart() {
      cartItemsContainer.innerHTML = cart.map(item => `
        <tr>
          <td>
            <div>${item.name}</div>
            <small class="text-muted">Stock: ${item.stock} ${item.unidad}</small>
          </td>
          <td>
