              ...
              <div class="card h-100 shadow-sm border-0">
                <div class="card-body text-center p-3">
                  ...
                  <p class="card-text mb-0">
                    <small class="text-muted"><?= $p->stock_inve ?> <?= esc($p->unidad ?? 'und') ?></small>
                  </p>
                   <p class="card-text mb-0">
                     <small class="text-muted">Lote: <?= esc($p->id) ?></small>
                  </p>
                </div>
              </div>
            </div>
    ...
    function renderProducts(filter = '') {
      ...
      productsGrid.innerHTML = paginated.map(p => `
        <div class="col-6 col-md-4 product-card" 
             ...
             data-name="${p.nombre}"
             data-price="${p.precio_contado}" 
             data-stock="${p.stock}"          // ✅ Ya es stock_inve
             data-unidad="${p.unidad}">
          <div class="card h-100 shadow-sm border-0 cursor-pointer">
            <div class="card-body text-center p-3">
              <div class="bg-light rounded-circle d-flex align-items-center justify-content-center mx-auto mb-2" 
                   style="width: 60px; height: 60px;">
                <span class="fw-bold text-success">${p.nombre.substring(0,2)}</span>
              </div>
              <h6 class="card-title fs-6 mb-1">${p.nombre}</h6>
              <p class="card-text mb-1">
                <span class="text-success fw-bold">Bs. ${parseFloat(p.precio_contado).toFixed(2)}</span>
              </p>
              <p class="card-text mb-0">
                <small class="text-muted">${p.stock} ${p.unidad}</small>
              </p>
            </div>
          </div>
        </div>
