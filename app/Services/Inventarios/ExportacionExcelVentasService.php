...
class ExportacionExcelVentasService
{
    public function exportar(string $fecha_inicio, string $fecha_fin, string $tipo): void
    {
        ...
        $reportData = $ventaModel->getDailySalesReportDataInve($fecha_inicio, $fecha_fin, $tipoConsulta);
        ...
        $filename = 'ventas_lacteos_' . $tipo . '_matrix_' . $fecha_inicio . '.xls';
        ...
        $totalCols = 1 + ($numProds * 2) + 2 - 1; // APELLIDOS + (Q+Bs)*prods + Nro.Venta + TOTAL - 1 (MergeAcross es 0-based)

        echo '<Worksheet ss:Name="VENTAS LACTEOS">';
        echo '<Table>';
        ...
        echo '<Row></Row>';
        echo '<Row ss:Height="22"><Cell ss:MergeAcross="' . $totalCols . '" ss:StyleID="header"><Data ss:Type="String">VENTAS ' . $tipoLabel . ' - TIENDA LACTEOS</Data></Cell></Row>';
        echo '<Row></Row>';
