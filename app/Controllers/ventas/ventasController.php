    ...
    public function index1()
{
    ...
    /**
     * Elimina la última venta del usuario hoy (módulo lácteos tienda).
     * Devuelve el stock original y hace hard-delete de detalles + soft-delete de la venta.
     */
    public function deleteUltimaVenta()
    {
        ...
        // Verificar que sea la última venta del usuario hoy en este módulo
        $ultima = $db->query("
            SELECT id FROM condoriri.ventas
            WHERE deleted_at IS NULL AND user_id = ? AND sucursal_id = ?
              AND DATE(created_at) = CURRENT_DATE
        ...
        try {
            ...
            // 2. Devolver stock a stock_sucursales
            foreach ($detalles as $det) {
                $db->query("
                    UPDATE condoriri.stock_sucursales SET stock = stock + ? WHERE id = ?
                ", [$det->cantidad, $det->stock_id]);
            }

            // 3. Hard-delete de detalles (uk_detalle_stock impide soft-delete + posible reinsert futuro)
            foreach ($detalles as $det) {
                $db->query("DELETE FROM condoriri.detalle_venta WHERE id = ?", [$det->id]);
            }
            ...
            $db->query("
                UPDATE condoriri.ventas SET deleted_at = NOW() WHERE id = ?
            ", [$ventaId]);
